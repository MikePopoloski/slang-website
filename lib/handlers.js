...
function handleCompilationRequest(req, res, next, opts) {
    ...
    logger.info('Compilation request', source, options);

    var args = options.split(' ').filter(e => e);
    args = ['source.sv', '--color-diagnostics=false'].concat(args);

    var promise;
    if (opts.localpath) {
        promise = fs.writeFile('source.sv', source).then(() => {
            return exec.execute(opts.localpath, args);
        });
    }
    else {
        promise = tmp.withFile(o => {
            ...
            return fs.writeFile(o.fd, source).then(() => {
                return exec.sandbox('/slangbin', args, {dockerArgs: [mountBin, mountSrc, '-t']});
            });
        })
    }
